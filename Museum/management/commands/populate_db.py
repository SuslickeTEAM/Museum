    ...
    def handle(self, *args, **kwargs):
        ...
        for name in category_names:
            category = Category.objects.create(
                title=name,
                image=self.get_dummy_image('category', (400, 300))
            )
        ...
        for i in range(5):
            Event.objects.create(
                image=self.get_dummy_image('event', (600, 400))
            )

        # Create some exhibits
        exhibit_titles = ['Dinosaur Fossil', 'Ancient Sculpture', 'Space Shuttle Model', 'Impressionist Painting', 'Medieval Armor']
